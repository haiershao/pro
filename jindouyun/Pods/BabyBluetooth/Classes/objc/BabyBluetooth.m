...
- (void)connectBaby:(BabyBluetooth *)baby peripheral:(CBPeripheral *)peripheral peripheralView:(NSString *)peripheralView{
    
    baby.having(peripheral).and.channel(peripheralView).then.connectToPeripherals().discoverServices().discoverCharacteristics().readValueForCharacteristic().discoverDescriptorsForCharacteristic().readValueForDescriptors().begin();
}
@end
